...
export default function RankPhase() {
  ...
  const numColor = (r: number) => r === 1 ? 'var(--rank-1)' : r === 2 ? 'var(--rank-2)' : r === 3 ? 'var(--rank-3)' : 'var(--text-tertiary)';
    ...
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '32px 24px' }}>
      ...
      {sorted.map((track: Track) => {
        ...
        return (
          ...
          >
            <div style={{ fontFamily: '"DM Mono", monospace', fontSize: numSize(rank), color: numColor(rank), minWidth: 36, textAlign: 'right' }}>#{rank}</div>
