...
import { fetchAllTracks, fetchPlaylists, fetchTrackDurations } from '../../utils/spotify';
...
export default function ImportPhase({ onSyncTrigger }: Props) {
  ...
  // durationMs 일괄 보강: 1-c 이전 레거시 곡(durationMs=0)을 배치로 메꾼다.
  // SortPhase의 쌍 단위 보강은 안전망으로 유지되지만, ImportPhase 진입 시
  // 한 번에 다 처리하는 편이 UX상 낫다.
  const duraBackfillAttemptedRef = useRef<Set<string>>(new Set());
  useEffect(() => {
    if (!isLoggedIn()) return;
    const needIds = state.tracks
      .filter(t => !t.durationMs)
      .map(t => t.id)
      .filter(id => !duraBackfillAttemptedRef.current.has(id));
    if (!needIds.length) return;

    // 이번 시도 대상을 미리 기록해 재진입 방지 (부분 실패해도 재시도 안 함)
    needIds.forEach(id => duraBackfillAttemptedRef.current.add(id));

    (async () => {
      try {
        const details = await fetchTrackDurations(needIds, getToken);
        if (details.length) {
          dispatch({ type: 'ENRICH_TRACKS', payload: details });
        }
      } catch (e) {
        console.warn('[ImportPhase] durationMs 일괄 보강 실패:', e);
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [state.tracks.length, isLoggedIn()]);
  ...
  const S = {
    card: { background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: 24, marginBottom: 16 } as React.CSSProperties,
    btn: (variant: 'green' | 'outline' | 'ghost') => ({
      display: 'inline-flex', alignItems: 'center', gap: 6,
      padding: '9px 20px', borderRadius: 8, cursor: 'pointer',
      fontFamily: '"DM Sans", sans-serif', fontSize: '0.875rem', fontWeight: 600,
      transition: 'all 0.15s', whiteSpace: 'nowrap' as const,
      ...(variant === 'green' ? { background: 'var(--accent)', color: '#000', border: '1px solid var(--accent)' }
        : variant === 'outline' ? { background: 'transparent', color: 'var(--text-secondary)', border: '1px solid var(--border)' }
  ...
  if (!isLoggedIn()) {
    return (
      <div style={{ textAlign: 'center', padding: '64px 24px' }}>
        ...
        <p style={{ marginTop: 16, fontSize: '0.78rem', color: 'var(--text-tertiary)' }}>설정(⚙)에서 Spotify Client ID와 Supabase 정보를 먼저 입력하세요</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            {user?.imageUrl
              ...
              : <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--bg-sub)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)' }}>👤</div>
            }
            <div>
              <div style={{ fontWeight: 600, fontSize: '0.95rem' }}>{user?.display_name || '—'}</div>
              <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: 2 }}>Spotify 연결됨</div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <div style={{ fontSize: '0.78rem', color: 'var(--text-tertiary)', fontFamily: '"DM Mono", monospace', display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: cfg.supabaseUrl ? 'var(--accent)' : 'var(--text-tertiary)' }} />
      ...
      {state.pendingNewTracks.length > 0 && (
        <div style={{ background: 'var(--accent-soft)', border: '1.5px solid var(--accent-border)', borderRadius: 12, padding: '14px 18px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ fontFamily: '"DM Mono", monospace', fontSize: '1.4rem', fontWeight: 700, color: 'var(--accent)', flexShrink: 0 }}>{state.pendingNewTracks.length}</div>
          <div style={{ flex: 1, fontSize: '0.88rem', lineHeight: 1.5 }}>
            <strong>신규 곡이 감지되었습니다</strong><br />
            <span style={{ fontSize: '0.82rem', color: 'var(--text-secondary)' }}>{state.pendingNewTracks.slice(0, 3).map(t => `${t.name} — ${t.artists[0]}`).join(' / ')}{state.pendingNewTracks.length > 3 ? ` 외 ${state.pendingNewTracks.length - 3}곡` : ''}</span>
          </div>
...
{state.pendingRemovedIds.length > 0 && (
  <div style={{ background: 'var(--danger-soft)', border: '1.5px solid var(--danger-border)', borderRadius: 12, padding: '14px 18px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
    <div style={{ fontFamily: '"DM Mono", monospace', fontSize: '1.4rem', fontWeight: 700, color: 'var(--danger)', flexShrink: 0 }}>{state.pendingRemovedIds.length}</div>
    <div style={{ flex: 1, fontSize: '0.88rem', lineHeight: 1.5 }}>
    ...
    <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
      <button onClick={() => dispatch({ type: 'APPLY_REMOVAL' })}
        style={{ padding: '7px 14px', borderRadius: 8, border: '1px solid var(--danger-border)', background: 'transparent', color: 'var(--danger)', cursor: 'pointer', fontFamily: '"DM Sans", sans-serif', fontSize: '0.82rem', fontWeight: 600 }}>
        로컬에서도 삭제
      </button>
      <button onClick={() => dispatch({ type: 'DISMISS_REMOVAL' })}
        style={{ padding: '7px 14px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-secondary)', cursor: 'pointer', fontFamily: '"DM Sans", sans-serif', fontSize: '0.82rem', fontWeight: 600 }}>
        유지
      </button>
    </div>
  </div>
      ...
      {/* Source picker */}
      <div style={S.card}>
        <div style={{ fontSize: '0.78rem', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-tertiary)', marginBottom: 16, fontFamily: '"DM Mono", monospace' }}>음악 소스 선택</div>

        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          {(['liked', 'playlist'] as const).map(t => (
            <button key={t} onClick={t === 'liked' ? () => setSourceTab('liked') : switchToPlaylist}
              style={{ padding: '7px 16px', borderRadius: 8, border: '1.5px solid ' + (sourceTab === t ? 'var(--accent)' : 'var(--border)'), background: sourceTab === t ? 'var(--accent-soft)' : 'transparent', color: sourceTab === t ? 'var(--accent)' : 'var(--text-secondary)', fontFamily: '"DM Sans", sans-serif', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer' }}>
        ...
        {sourceTab === 'liked' && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
            <div>
              ...
              <div style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', marginTop: 4 }}>Spotify에서 실시간 불러오기</div>
            </div>
        ...
        {sourceTab === 'playlist' && (
          <div>
            {plLoading && <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--text-secondary)', fontSize: '0.88rem', padding: '16px 0' }}><span className="spinner" /> 플레이리스트 목록 로딩 중…</div>}
            {!plLoading && playlists.length === 0 && <div style={{ color: 'var(--text-secondary)', fontSize: '0.88rem', padding: '12px 0' }}>플레이리스트가 없습니다</div>}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 360, overflowY: 'auto', marginTop: 12 }}>
              {playlists.map(pl => (
                <button key={pl.id} onClick={() => setSelectedPl(pl)}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', borderRadius: 10, border: '1.5px solid ' + (selectedPl?.id === pl.id ? 'var(--accent)' : 'var(--border)'), background: selectedPl?.id === pl.id ? 'var(--accent-soft)' : 'transparent', cursor: 'pointer', textAlign: 'left' }}>
                  <img src={pl.images?.[0]?.url || ''} alt="" style={{ width: 44, height: 44, borderRadius: 6, objectFit: 'cover', background: 'var(--bg-sub)', flexShrink: 0 }} onError={e => (e.currentTarget.style.display = 'none')} />
                  <div>
                    <div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-primary)' }}>{pl.name}</div>
                    <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', marginTop: 2 }}>{pl.tracks?.total ?? 0}곡</div>
                  </div>
                </button>
              ))}
            </div>
      ...
      {/* Cloud */}
      <div style={S.card}>
        <div style={{ fontSize: '0.78rem', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-tertiary)', marginBottom: 16, fontFamily: '"DM Mono", monospace' }}>클라우드 저장</div>
        {cloudInfo?.exists ? (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div>
              <div style={{ fontSize: '0.9rem', fontWeight: 600 }}>저장된 세션</div>
              <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: 3 }}>{cloudInfo.trackCount}곡 · {cloudInfo.updatedAt ? new Date(cloudInfo.updatedAt).toLocaleString('ko-KR') : '—'}</div>
            </div>
          ...
          <div style={{ fontSize: '0.88rem', color: 'var(--text-secondary)' }}>{cfg.supabaseUrl ? '저장된 세션이 없습니다' : 'Supabase를 설정하면 클라우드 저장이 활성화됩니다'}</div>
        )}
      </div>

      {/* Resume session */}
      {state.tracks.length > 0 && (
        <div style={{ ...S.card, border: '1px solid var(--accent-border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div>
              <div style={{ fontSize: '0.95rem', fontWeight: 600 }}>현재 세션 이어하기</div>
              <div style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', marginTop: 3 }}>{state.tracks.length}곡 · 비교 {state.compCount}회</div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
