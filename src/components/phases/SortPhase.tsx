  ...
  return (
    <div style={{ position: 'relative' }}>
      <button onClick={() => setOpen(v => !v)}
        style={{ width: 20, height: 20, borderRadius: '50%', border: '1px solid var(--border-strong)', background: 'transparent', color: 'var(--text-tertiary)', fontSize: '0.75rem', fontFamily: '"DM Mono", monospace', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
        aria-label="키보드 단축키 보기">?</button>
      {open && (
        <div style={{ position: 'absolute', right: 0, top: 28, zIndex: 50, background: 'var(--bg-sub)', border: '1px solid var(--border-strong)', borderRadius: 12, padding: 12, fontSize: '0.75rem', fontFamily: '"DM Mono", monospace', color: 'var(--text-secondary)', whiteSpace: 'nowrap', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
          {isFiveStep
...
function CmpCard({ track, onClick, locked }: CmpCardProps) {
  return (
    <div onClick={() => !locked && onClick()}
      style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border)', borderRadius: 18, overflow: 'hidden', cursor: locked ? 'default' : 'pointer', transition: 'all 0.18s', display: 'flex', flexDirection: 'column' }}
      onMouseEnter={e => { if (!locked) { e.currentTarget.style.borderColor = 'var(--accent)'; e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 0 0 1px var(--accent), 0 8px 32px var(--accent-soft)'; } }}
    ...
    >
      ...
      <div style={{ padding: 16, flex: 1 }}>
        ...
        <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: 8 }}>{track.artists.join(', ')}</div>
        <div style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.78rem', color: 'var(--text-tertiary)', display: 'flex', justifyContent: 'space-between' }}>
          <span>ELO</span>
          <span style={{ color: 'var(--accent)', fontSize: '0.85rem' }}>{Math.round(track.rating)}</span>
        </div>
      </div>
    </div>
  );
...
export default function SortPhase({ player }: Props) {
  ...
  const rsiColor = rsi !== null ? (rsi < 10 ? 'var(--accent)' : rsi < 25 ? 'var(--warning)' : 'var(--danger)') : 'var(--danger)';
    ...
    <div style={{ maxWidth: '60%', margin: '0 auto', height: 'calc(100vh - 56px)', padding: 24, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, gap: 16, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          ...
          <div style={{ padding: '4px 12px', borderRadius: 20, fontSize: '0.78rem', fontWeight: 600, fontFamily: '"DM Mono", monospace', ...(isFiveStep ? { background: 'var(--accent-soft)', color: 'var(--accent)', border: '1px solid var(--accent-border)' } : { background: 'var(--danger-soft)', color: 'var(--danger-text)', border: '1px solid var(--danger-border)' }) }}>
            {isFiveStep ? '5단계 선택' : '이진 선택'}
          </div>
          <button onClick={handleSaveAndExit} style={{ padding: '5px 12px', borderRadius: 20, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-tertiary)', cursor: 'pointer', fontSize: '0.78rem', whiteSpace: 'nowrap' }}>
            저장 후 나가기
          </button>
        </div>
      </div>
      ...
      {/* RSI bar */}
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '12px 16px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
        <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', whiteSpace: 'nowrap' }}>수렴 (RSI)</span>
        <div style={{ flex: 1, background: 'var(--bg-section)', height: 6, borderRadius: 3, overflow: 'hidden' }}>
        ...
        <span style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)', whiteSpace: 'nowrap' }}>목표: &lt;10점</span>
      </div>
        ...
        {!isFiveStep ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 24, alignItems: 'stretch', height: '100%' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7, alignItems: 'center', justifyContent: 'center', padding: '0 10px' }}>
              <div style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.75rem', color: 'var(--text-tertiary)', textAlign: 'center', marginBottom: 4, letterSpacing: '0.1em' }}>A &nbsp; VS &nbsp; B</div>
              {fiveStepBtns.map(b => (
                <button key={b.score} onClick={() => handleChoose(b.score)} disabled={isChoosing}
                  style={{ width: 168, padding: '10px 14px', borderRadius: 10, border: '1px solid ' + (b.side === 'left' ? 'var(--tier-1-border)' : b.side === 'right' ? 'var(--tier-2-border)' : 'var(--border)'), background: 'var(--bg-sub)', color: 'var(--text-primary)', fontFamily: '"DM Sans", sans-serif', fontSize: '0.82rem', fontWeight: 500, cursor: isChoosing ? 'not-allowed' : 'pointer', transition: 'all 0.15s', display: 'flex', alignItems: 'center', justifyContent: 'space-between', ...(b.side === 'left' ? { borderLeft: '3px solid var(--tier-1)' } : b.side === 'right' ? { borderRight: '3px solid var(--tier-2)', borderLeft: 'none' } : {}), opacity: isChoosing ? 0.5 : 1 }}
                >
                  <span>{b.label}</span>
                  <span style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.7rem', color: 'var(--text-tertiary)' }}>{b.key}</span>
                </button>
              ))}
            </div>
      ...
      {/* Hint */}
      {!isFiveStep && <div style={{ textAlign: 'center', fontSize: '0.8rem', color: 'var(--text-tertiary)', marginBottom: 16, flexShrink: 0 }}>클릭하거나 <kbd style={{ fontFamily: '"DM Mono", monospace', background: 'var(--bg-sub)', padding: '1px 6px', borderRadius: 4, fontSize: '0.75rem' }}>←</kbd> / <kbd style={{ fontFamily: '"DM Mono", monospace', background: 'var(--bg-sub)', padding: '1px 6px', borderRadius: 4, fontSize: '0.75rem' }}>→</kbd> 키로 선택</div>}

      {/* Spotify embeds + SDK play buttons */}
      <div style={{ flexShrink: 0, marginBottom: 20 }}>
        {!player.sdkFailed && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
            <button onClick={() => player.play(A.uri)} style={{ padding: '8px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              ▶ A 재생 (전곡)
            </button>
            <button onClick={() => player.play(B.uri)} style={{ padding: '8px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              ▶ B 재생 (전곡)
            </button>
          </div>
        )}
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', flexShrink: 0 }}>
        <button onClick={() => dispatch({ type: 'SET_PHASE', payload: 'rank' })} style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: '0.8rem' }}>🏆 현재 랭킹 보기</button>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={async () => { await saveToSupabase({ tracks, compCount, rsiDeltas, currentSource }, cfg); showToast('☁️ 저장됨'); }} style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: '0.8rem' }}>☁️ 저장</button>
          <button onClick={() => dispatch({ type: 'SET_PHASE', payload: 'rank' })} style={{ padding: '6px 14px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#000', fontWeight: 600, cursor: 'pointer', fontSize: '0.8rem' }}>완료 →</button>
        </div>
      </div>
    </div>
  );
